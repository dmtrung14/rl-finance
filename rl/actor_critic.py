...
class ActorCritic(nn.Module):
    ...
    def __init__(self,  num_obs,
                        num_critic_obs,
                        num_scan, # this is currently wrong
                        num_actions,
        ...
        self.flat_terrain = flat_terrain
        ...
        actor_layers = []
        if flat_terrain:
            self.scan_encoder_output_dim = 0
        actor_layers.append(nn.Linear(num_obs + self.scan_encoder_output_dim, actor_hidden_dims[0]))
        ...
        if not flat_terrain:
            print(f"Scan encoder MLP: {self.scan_encoder}")
    ...
    def encode_obs(self, obs):
        ...
        obs = torch.cat((obs[:, :self.num_prop], scan_latent), dim=1)
        return obs
